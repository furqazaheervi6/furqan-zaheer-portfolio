...
5. **Spacing is native Tailwind scale, not old token numbers.** Use `p-4`,
   `gap-2`, `h-10`, `tablet:grid`. Do not use stale classes like `p-400`,
   `gap-200`, or `mt-300`.
...
| Border width | `border-q-thin`, `border-q-medium` |
...
Breakpoints are mobile-first: base, `tablet:`, `desktop:`, `wide:`.
...
| `Sidebar` | `@higgsfield/quanta/sidebar` | Product navigation rail — header switcher, sections, composite `Item` rows (start/meta/end), collapse |
...
2. **Use stable shell structure.** App-like products usually need a root shell:
   `min-h-dvh`, fixed or sticky header, optional sidebar, main work area, and
   constrained scroll regions. Avoid content floating in random page space.
