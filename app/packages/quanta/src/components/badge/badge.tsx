 ...
 * Purple are skewed uppercase caps; NBrand/NBlue are compact "new" markers.
 * Two sizes (Figma node 526:456): xs (default) and sm.
 */

export type BadgeVariant = 'blue' | 'lime' | 'pink' | 'purple' | 'nBrand' | 'nBlue'
...
export function Badge({ variant = 'blue', size = 'xs', text, className, children, ...props }: BadgeProps) {
  const content = children ?? text ?? (variant === 'nBrand' || variant === 'nBlue' ? 'new' : 'Tag')
    ...
    >
      <span className="q-badge-frame">
        <span className="q-badge-surface">
          <span className="q-badge-text">{content}</span>
        </span>
      </span>
    </span>
  )
