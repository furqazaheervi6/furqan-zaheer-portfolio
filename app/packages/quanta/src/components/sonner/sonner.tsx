...
const SWIPE: Record<SonnerPosition, ('up' | 'down' | 'left' | 'right')[]> = {
  'top-left': ['up', 'left'],
  'top-center': ['up'],
  'top-right': ['up', 'right'],
...
function Glyph({ variant }: { variant: SonnerVariant }) {
  const common = { viewBox: '0 0 20 20', fill: 'none', 'aria-hidden': true, className: 'size-5' } as const
  switch (variant) {
    case 'success':
      return <svg {...common}><circle cx="10" cy="10" r="8" stroke="currentColor" strokeWidth="1.5" /><path d="M6.5 10.2l2.2 2.3 4.8-5" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" /></svg>
    case 'error':
      return <svg {...common}><circle cx="10" cy="10" r="8" stroke="currentColor" strokeWidth="1.5" /><path d="M7 7l6 6M13 7l-6 6" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" /></svg>
    case 'warning':
      return <svg {...common}><path d="M10 2.5l7.5 13H2.5l7.5-13z" stroke="currentColor" strokeWidth="1.5" strokeLinejoin="round" /><path d="M10 8v3.5M10 14h.01" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" /></svg>
    case 'info':
      return <svg {...common}><circle cx="10" cy="10" r="8" stroke="currentColor" strokeWidth="1.5" /><path d="M10 9v4.5M10 6.5h.01" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" /></svg>
    case 'loading':
      return <svg {...common} className="size-5 q-sonner-spinner"><circle cx="10" cy="10" r="8" stroke="currentColor" strokeOpacity="0.25" strokeWidth="1.5" /><path d="M18 10a8 8 0 0 0-8-8" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" /></svg>
    default:
      return null
  }
}
...
function SonnerToast({ toast: t, swipe }: { toast: Primitive.Root.ToastObject<SonnerData>, swipe: ('up' | 'down' | 'left' | 'right')[] }) {
  ...
  return (
    <Primitive.Root toast={t} swipeDirection={swipe} className={cx('q-sonner', VARIANT_CLASS[variant])}>
      ...
      <Primitive.Close className="q-sonner-close" aria-label="Dismiss"><CloseIcon className="size-4" /></Primitive.Close>
    </Primitive.Root>
  )
}

function ToastList({ swipe }: { swipe: ('up' | 'down' | 'left' | 'right')[] }) {
