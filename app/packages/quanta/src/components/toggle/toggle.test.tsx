...
describe('<Toggle>', () => {
  ...
  it('applies the md size classes by default', () => {
    ...
    expect(screen.getByRole('button', { name: 'M' })).toHaveClass('h-10', 'px-3')
  })

  it('maps size to its token sizing classes', () => {
    const { rerender } = render(<Toggle size="sm">S</Toggle>)
    expect(screen.getByRole('button', { name: 'S' })).toHaveClass('h-8', 'px-2')

    rerender(<Toggle size="lg">L</Toggle>)
    expect(screen.getByRole('button', { name: 'L' })).toHaveClass('h-12', 'px-4')
  })
  ...
  it('wires the slot color custom properties from the color prop', () => {
    ...
    expect(btn.style.getPropertyValue('--_c')).not.toBe('')
  })
