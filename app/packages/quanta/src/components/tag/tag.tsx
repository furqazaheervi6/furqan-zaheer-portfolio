...
import { CloseIcon } from '../menu/icons.tsx'
...
export function Tag({ color = 'neutral', start, end, onRemove, removeLabel = 'Remove', className, style, children, ...props }: TagProps) {
  return (
    <span
      ...
      className={cx(
        'inline-flex max-w-full items-center gap-1 rounded-md px-2 py-0.5 align-middle text-q-caption-sm-medium',
        'q-slot-bg-10 q-slot-text',
        className,
      )}
      {...props}
    >
      {start != null ? <span className="inline-flex shrink-0 items-center [&_svg]:size-3">{start}</span> : null}
      <span className="truncate">{children}</span>
      {end != null ? <span className="inline-flex shrink-0 items-center [&_svg]:size-3">{end}</span> : null}
      {onRemove
        ? (
            <button
              type="button"
              ...
              className={cx(
                '-mr-0.5 inline-flex size-4 shrink-0 cursor-pointer items-center justify-center rounded-sm outline-none transition-colors',
                'hover:q-slot-bg-20 focus-visible:ring-2 focus-visible:q-slot-ring-40 [&_svg]:size-3',
              )}
            >
              <CloseIcon />
            </button>
