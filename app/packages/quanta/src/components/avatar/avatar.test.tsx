...
describe('<Avatar>', () => {
  ...
  it('scales the online dot to the Figma status size for xl avatars', () => {
    ...
    expect(screen.getByRole('img', { name: 'online' })).toHaveClass('size-2')
  })
  ...
  it('renders a dashed placeholder with no colored fill', () => {
    const { container } = render(<Avatar variant="dashed" alt="Add" />)
    // Dashed ring is an SVG stroke (controllable dash length), not border-dashed.
    expect(container.querySelector('svg.q-avatar-dash .q-avatar-dash-circle')).not.toBeNull()
