...
export type SwitchProps = Omit<ComponentProps<typeof Primitive.Root>, 'size'> & {
  /** Slot color — sets `--_c`; every surface derives from it. Default 'brand'. */
  color?: SlotColor
  size?: SwitchSize
}
...
export function SwitchLabel({
  ...
  const textNode = (
    <span className="q-switch-label-text">
      <span className="q-switch-label-title">{children ?? label}</span>
      {description ? <span className="q-switch-label-description">{description}</span> : null}
    </span>
  )
