...
export function NotFound({ icon, title, subtitle, actions, size = 'md', variant = 'plain', className, children, render, ...props }: NotFoundProps) {
  return useRender({
    render,
    defaultTagName: 'div',
    props: {
        ...
        <>
          ...
          {(title != null || subtitle != null) ? (
            <span className="q-not-found-text">
              {title != null ? <span className="q-not-found-title">{title}</span> : null}
              {subtitle != null ? <span className="q-not-found-subtitle">{subtitle}</span> : null}
            </span>
          ) : null}
