...
import { normalizeProductJob } from './job-response-normalize'
...
interface ProdJob {
  id: string
  status: string
  results?: { raw?: { url?: string, thumbnail_url?: string }, min?: { url?: string, thumbnail_url?: string }, thumbnail?: { url?: string }, thumbnail_url?: string }
...
function mapJob(job: ProdJob, set?: ProdJobSet): JobResponse & { job_set_type?: string } {
  // Legacy serializers leak raw 'waiting' and product serializers vary between
  // nested `results.*.url` and flat `*_url` fields. Keep that normalization in
  // one adapter helper so WFP and fnf-web reads produce the same SDK shape.
  return normalizeProductJob(job as unknown as Record<string, unknown>, set as unknown as Record<string, unknown> | undefined)
}
